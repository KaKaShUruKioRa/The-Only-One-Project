...
local function initialize_mouse_control_features(game)
  ...
  function mouse_control:on_update()
    ...
    if is_mouse_button_pushed then
      ...
      local threshold = deadzone_ray
      local quest_width, quest_height = sol.video.get_quest_size()
      if mouse_y >= quest_height - 2 then
        threshold = 1
      end

      -- Compare the position of the hero and the mouse
      -- and simulate the appropriate command for each direction.
      self:update_direction("right", mouse_x > hero_x + threshold)
      self:update_direction("up", mouse_y < hero_y - threshold)
      self:update_direction("left", mouse_x < hero_x - threshold)
      self:update_direction("down", mouse_y > hero_y + threshold)
    else
